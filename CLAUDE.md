...
- **Retain cache** (shardedglobalheap.h): purging EVERY empty superblock discards exactly the pages that are about to be reallocated — it costs ~27% on any workload that cycles memory, because every reuse re-faults. A bounded budget of empty superblocks (default 64MB, `HOARD_RETAIN_MB`; 0 restores purge-everything) is therefore held **unpurged**; beyond it, purging resumes. Churn that fits the budget is ~24% faster, and a program that frees a large working set still returns nearly all of it (measured: 496MB of 690MB returned, vs mimalloc's 76MB — mimalloc's fast bulk reuse is largely *because* it keeps the pages). Bookkeeping is one flag in the superblock header (fits existing padding; the header stays 256 bytes) plus one atomic byte counter; a drifting counter can only make Hoard purge MORE, never less, so RSS stays bounded.
- **Purge-on-empty** (shardedglobalheap.h): beyond the retain budget, a completely-empty superblock reaching the global heap has its data pages discarded via `MADV_FREE_REUSABLE` (macOS; `MADV_FREE`/`MADV_DONTNEED` on Linux, `DiscardVirtualMemory` on Windows — see util/purge.h). `clear()` first resets the freelist to bump-pointer mode so no allocator metadata lives in purged pages. Plain `MADV_FREE` does NOT lower RSS/footprint numbers on macOS until memory pressure; `MADV_FREE_REUSABLE` does.
